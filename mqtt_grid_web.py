...
def handle_release_complete(data):
    ...
    if user_id in memory_data_store:
        ...
        # Idempotency guard: ignore duplicate release_complete events
        if not memory_data.release_pending:
            logging.debug(f'[RELEASE] Ignoring duplicate release_complete for {user_id} (release_pending already False)')
            return
...
def _setup_mqtt_client(user_id):
    ...
    # Always reset Virtual_List on login to prevent stale state from previous day
    memory_data_store[user_id] = Virtual_List(user_id=user_id)
    logging.info(f"[MQTT-SETUP] Reset Virtual_List for user {user_id}")

    # Track session start time for auto-logout
    with _session_lock:
        _user_sessions[user_id] = (datetime.now(), class_code)
...
memory_data_store = {}
# Track login timestamps for auto-logout {user_id: (login_time, class_code)}
_user_sessions = {}
...
# ---------------------------------------------------------------
# Auto-logout: force-clean sessions older than SESSION_MAX_AGE_HOURS
# ---------------------------------------------------------------
def _force_logout_user(user_id, class_code):
    """Clean up MQTT client, memory data, and active class codes for a user."""
    try:
        if user_id in mqtt_clients:
            try:
                mqtt_clients[user_id].loop_stop()
                if mqtt_clients[user_id].is_connected():
                    ...
                    mqtt_clients[user_id].unsubscribe("IQRSend")
                    mqtt_clients[user_id].unsubscribe(f"{TOPIC_PREFIX}test")
                    mqtt_clients[user_id].disconnect()
                del mqtt_clients[user_id]
            except Exception as e:
                logging.error(f"[AUTO-LOGOUT] Error cleaning MQTT client for {user_id}: {e}")
        ...
        logging.info(f"[AUTO-LOGOUT] Force-logged out user {user_id} (classCode={class_code})")
    except Exception as e:
        logging.error(f"[AUTO-LOGOUT] Error during force logout of {user_id}: {e}")

def _session_cleanup_loop():
    """Background thread that checks for expired sessions every 5 minutes."""
    import threading
    while True:
        try:
            now = datetime.now()
            expired = []
            with _session_lock:
                for user_id, (login_time, class_code) in _user_sessions.items():
                    age_hours = (now - login_time).total_seconds() / 3600
                    if age_hours >= SESSION_MAX_AGE_HOURS:
                        expired.append((user_id, class_code))

                for user_id, class_code in expired:
                    del _user_sessions[user_id]

            for user_id, class_code in expired:
                _force_logout_user(user_id, class_code)
                # Notify the browser so it redirects to login
                try:
                    socketio.emit('session_expired', {'reason': 'Session expired after 2 hours'}, room=user_id)
                except Exception:
                    pass

        except Exception as e:
            logging.error(f"[AUTO-LOGOUT] Error in cleanup loop: {e}")

        time.sleep(300)  # Check every 5 minutes

_cleanup_thread = __import__('threading').Thread(target=_session_cleanup_loop, daemon=True)
_cleanup_thread.start()
logging.info(f"[AUTO-LOGOUT] Session cleanup thread started (max age: {SESSION_MAX_AGE_HOURS}h)")
