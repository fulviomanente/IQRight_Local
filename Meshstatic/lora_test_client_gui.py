        ...
        try:
            while sending:
                ...
                with self.lora_lock:
                    ...
                    if not self.rfm9x.send_with_ack(bytes(payload, "UTF-8")):
                        self.log_message(f"Error sending to Server - attempt {cont}", 'WARNING')
                        self.button_status.set(f"Error - {cont}")
                    else:
                        self.log_message(f"Info sent to Server - attempt {cont}", 'INFO')
                        self.button_status.set(f"Sent - {cont}")

                        # Now wait for response like scanner_queue.py's lora_receiver
                        if self.lora_receiver():
                            self.success_count += 1
                            sending = False
                            self.button_status.set("Ready")

                            # Update RSSI/SNR display after successful response
                            self.after(0, self.update_rssi_snr_display)
                            return True

                    # Check timeout - scanner_queue.py uses 5 seconds
