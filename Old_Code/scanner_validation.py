...
class App(tk.Tk):
    def __init__(self):
        ...
        btn_addBreak = tk.Button(master=barFrame, text="Problem", font=("Arial", 16), height=12, fg="blue",
                                 command=self.breakQueue).pack(fill='both', expand=True, side=tk.LEFT)
        btn_removeLast = tk.Button(master=barFrame, text="Test", height=12, font=("Arial", 16), fg="blue",
                                   command=self.releaseQueue).pack(fill='both', expand=True, side=tk.LEFT)
        btn_undo = tk.Button(master=barFrame, text="N/A", height=12, font=("Arial", 16), fg="blue",
    ...
    def breakQueue(self):
        self.sheet.insert_row(['PROBLEM FOUND', '', ''], redraw=True)
        ...
        while self.queue.qsize():
            try:
                ...
                if value:
                    ...
                    #    header = True
                    #    while header == True:
                    #        if len(new) > position + 2:
                    #            if new[init:position] == "31":
                    #                position = position + 2
                    #                init = init + 2
                    #            else:
                    #                position = position - 2
