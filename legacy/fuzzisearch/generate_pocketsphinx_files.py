...
def generate_grammar(unique_firsts, unique_lasts, full_names, output_path):
    """Generate a JSGF grammar file for PocketSphinx — LAST NAMES ONLY."""
    last_patterns = set()
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        f.write("public <lastname> = " + "\n                   | ".join(sorted(last_patterns)) + ";\n")

    print(f"\nGrammar generation summary (LAST NAMES ONLY):")
...
def main():
    ...
    all_tokens = sorted(set(unique_lasts))
    print(f"  Total unique last name tokens: {len(all_tokens)}")
