#!/usr/bin/env python3
"""
LoRa Test Client with GUI - Using refactored single-receiver architecture
...
class LoRaTestClientGUI(tk.Tk):
    def __init__(self):
        ...
        self.title("LoRa Test Client (NEW)")
        ...
        # Initialize LoRa FIRST (before GPIO setup)
        self.rfm9x = None
        self.node = None
        ...
        # GPIO Setup - AFTER LoRa initialization
        self.setup_gpio()
        ...
        if self.rfm9x:
            ...
            def on_msg(pkt, rssi):
                ...
                print(f"[SCANNER {self.node.node_id if self.node else 'N/A'}] RX src={pkt['src']} seq={pkt['seq']} rssi={rssi} payload={pkt['payload']}")
    ...
    def setup_gui(self):
        ...
        title_label = tk.Label(top_frame, text="LoRa Test Client (NEW)",
                               font=("Arial", 24, "bold"),
                               fg="white", bg='#2196F3')
    ...
    def setup_logging(self):
        ...
        log_filename = "lora_test_client_gui_new.log"
    ...
    def init_lora(self):
        ...
        try:
            with self.lora_lock:
                ...
                # Set parameters
                if self.tx_power.get() != 23:
                    self.rfm9x.tx_power = self.tx_power.get()
                if self.spreading_factor.get() != 7:
    ...
    def reinit_lora(self):
        ...
        # Stop the old node
        if self.node:
            self.node.stop_receiver()
            self.node = None

        # Reinitialize
        self.init_lora()

        # Create new node if LoRa init succeeded
        if self.rfm9x:
            self.node = MeshNode(self.rfm9x, node_id=RFM9X_NODE)
            ...
            self.node.on_message = on_msg
            self.log_message("LoRa reinitialized successfully", 'SUCCESS')
        else:
            self.log_message("LoRa reinitialization failed", 'ERROR')
    ...
    def send_message(self):
        """Send a single message using new architecture"""
        self.log_message("send message", 'INFO')

        if not self.rfm9x or not self.node:
            self.log_message("LoRa not initialized", 'ERROR')
            return False
        ...
        try:
            self.log_message("Call send_request...", 'INFO')
            ...
            if self.node.send_request(RFM9X_DESTINATION, payload, await_ack=True):
                ...
                if self.node.full_response:
                    for i, line in enumerate(self.node.full_response):
                        self.log_message(f"Response {i+1}: {line}", 'SUCCESS')
                    self.success_count += 1
                else:
                    self.log_message("No follow-up responses received", 'INFO')
                    self.success_count += 1

                self.button_status.set("Success")
                self.after(1000, lambda: self.button_status.set("Ready"))
                return True
            else:
                self.log_message("Request failed - no ACK or incomplete response", 'ERROR')
                self.fail_count += 1
                self.button_status.set("Failed")
                self.after(1000, lambda: self.button_status.set("Ready"))
                return False
    ...
    def button_monitor_loop(self):
        ...
        while self.is_running:
            try:
                # Ensure GPIO is configured
                try:
                    # Check if GPIO is still accessible
                    pressed = GPIO.input(GPIO_BUTTON_PIN)
                except RuntimeError:
                    # GPIO not configured, try to reconfigure
                    GPIO.setmode(GPIO.BCM)
                    GPIO.setup(GPIO_BUTTON_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
                    pressed = GPIO.input(GPIO_BUTTON_PIN)
                ...
                if pressed == 0 and not button_pressed:
                    ...
                    if self.rfm9x and self.node:
                        ...
                        self.log_message("LoRa not ready", 'WARNING')
                ...
                break  # Exit the loop if there's an unrecoverable error
